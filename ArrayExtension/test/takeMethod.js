...
describe('take Method', function() {
    ...
    describe('when is called', function() {
        it('should return the correct number of objects that satisfy the constraint', function() {
            babes.take(3, function(x) {
                    return x.sex == 'm';
                })
                .each(function(x) {
                    bufferTest.push(x.name)
                });
            expect(bufferTest).to.eql(['Tomas', 'Horhe', 'Arturo']);
        });
    });
    ...
    describe('when is called', function() {
        it('should be able to read null values', function() {
            babes.take(1, function(x) {
                    return x.sex == null;
                })
                .each(function(x) {
                    bufferTest.push(x.name)
                });
            expect(bufferTest).to.eql(['Carlos']);
        });
    });
});
